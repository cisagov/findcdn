...
def test_domainpot_init():
    ...
    for i in range(len(domains)):
        assert pot.domains[i].url == domains[i], "%s is not %s" % (pot.domains[i].url,domains[i])
        for j in range(len(domains)):
            if j == i:
                continue
            assert id(pot.domains[i].cdns) != id(pot.domains[j].cdns), "Domain obect %s shares address with Domain object %s" % (i,j)
...
def test_grab_cdn():
    ...
    # Assertions
    assert frontable[domains[0]] == ['.cloudflare.com'], "%s is not %s." % (frontable[domains[0]],['.cloudflare.com'])
    assert frontable[domains[1]] == ['.cloudfront.net', '.awsdn'], "%s is not %s." % (frontable[domains[1]], ['.cloudfront.net', '.awsdn'])
    assert frontable[domains[2]] == ['.cloudflare.com'], "%s is not %s." % (frontable[domains[2]], ['.cloudflare.com'])
    assert frontable[domains[3]] == ['.cloudflare.com'], "%s is not %s." % (frontable[domains[3]], ['.cloudflare.com'])
